 ...
 */
function normalizeLastPost(value: string | undefined): string | undefined {
...
async function fetchAllAccounts(names: string[]): Promise<Map<string, HiveAccount>> {
  ...
  for (let i = 0; i < batches.length; i += BATCH_CONCURRENCY) {
    ...
    const settled = await Promise.allSettled(slice.map(fetchAccountBatch));
    for (const result of settled) {
      if (result.status !== "fulfilled") continue;
      for (const account of result.value) accounts.set(account.name, account);
    }
  }
  return accounts;
}
 ...
 */
export const getSkaterDirectory = unstable_cache(
  async (): Promise<DirectoryData> => {
    try {
      return await buildDirectory();
    } catch (error) {
      console.error("[skaters] failed to build directory:", error);
      return { skaters: [], countries: [], generatedAt: new Date().toISOString() };
    }
  },
  ["skaters-directory-v1"],
